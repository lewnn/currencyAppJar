...
public class ChainFlatMapFuncV3 implements FlatMapFunction<Map, RowData> {
    ...
    public ChainFlatMapFuncV3(List<DataTypeProcess> dataTypeInfo,
                              DataType[] types,
                              int timePrecision,
                              int timeZone,
                              int debeziumTimeZone,
            ...
            case "r":
                ...
                for (int i = 0; i < dataTypeInfo.size(); i++) {
                    ...
                    if (colName.equalsIgnoreCase(updateTimeColName)) {
                        TimestampData updateTime = (TimestampData) DorisConvertValue.convertValue(after.get(colName), types[i], dataTypeInfo.get(i).getPrecision(), timeZone);
                        newRow.setField(dataTypeInfo.size(), StringData.fromString(DorisConvertValue.formatDate(updateTime.toLocalDateTime())));
                    }
                }
                newRow.setField(dataTypeInfo.size() + 1, StringData.fromString(CdcConstant.MAX_DATE_STR));
                out.collect(newRow);
                break;
            case "d":
                ...
                for (int i = 0; i < dataTypeInfo.size(); i++) {
                    ...
                    if (colName.equalsIgnoreCase(updateTimeColName)) {
                        TimestampData updateTime = (TimestampData) DorisConvertValue.convertValue(
                                ...
                                dataTypeInfo.get(i).getPrecision(),
                                timeZone);
                        newRow.setField(dataTypeInfo.size() + 1, StringData.fromString(DorisConvertValue.formatDate(updateTime.toLocalDateTime())));
                        endTimeIsSet = true;
                    }
                }
                if (!endTimeIsSet) {
